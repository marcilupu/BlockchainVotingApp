...
contract Registration {
    ...
    // add a new Event
    event NewRegistration(address sender, uint registeredHash);

    mapping(uint256 => bool) public usersRegistered;

    constructor(){
        verifier = new Verifier();
    }
 
    function register(uint256 ax, uint256 ay, uint256 bx0, uint256 bx1, uint256 by0, uint256 by1, uint256 cx, uint256 cy, uint256 proofSha) public returns (bool){
        //todo: check the args
        require(!usersRegistered[proofSha], "The user has already registered");
        ...
        {
            usersRegistered[proofSha] = true;

            emit NewRegistration(msg.sender, proofSha);
